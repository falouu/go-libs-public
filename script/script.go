...
type Script interface {
	// directory where go script (with main() method) is located. Absolute path
	Dir() string
	// directory from where go script was called
	Cwd() string
	// run command selected by Kong CLI based on command line arguments
...
const (
	// actual current directory from the perspective of the user. This info is needed because when script is run using
	// 'go run' command, the current directory may be messed up
	APP_CWD InputEnvVar = "APP_CWD"
	// actual argument 0 - file path the user provided to run the script
	APP_ARG_0 InputEnvVar = "APP_ARG_0"
	// path to the script you want to use for subsequent runs (used for example to generate alias in
...
func initScript(options *Options) (Script, error) {
	...
	os.Unsetenv(string(APP_SCRIPTPATH))
	if appScriptpath == "" {
